...
class inventory_scm_dropship_report(models.AbstractModel):
    _inherit = 'account.report'
    _name = 'inventory.scm.dropship.report'
    _description = 'Inventory Dropship Report'
...
#    filter_all_entries = False
    ...
    def _get_templates(self):
        templates = super(inventory_scm_dropship_report, self)._get_templates()
    ...
    def _get_lines(self, options, line_id=None):
        ...
        for so in sale_order:
            do_no = do_receive_date = status = do_date = pod_date = purchase_order = origin = False
                    ...
                    for picking_line in picking.move_ids_without_package:
                        ...
                        if so.purchase_request_id:
                            for prl in so.purchase_request_id.line_ids:
                                if prl.product_id==picking_line.sale_line_id.product_id:
                                    for line in prl.purchase_lines:
                                        purchase_order = line.order_id
                                        if picking.origin!=so.name and picking.origin!=purchase_order.name:
                                        ...
                                        line = {
                                            ...
                                            'columns': [
                                                {'name': purchase_order.name or '', 'no_format_name': '', 'class': 'text-left'},
                                                ...
                                                {'name': do_date or '', 'no_format_name': '', 'class': 'text-left'}, #approved date
    ...
    def _get_sales_order(self, options):
        ...
        results = self.env['sale.order'].search([('state','not in',['draft','sent','cancel']),('date_order','>=',date_from),('date_order','<=',date_to),('route_id.is_dropship','=',True)], order="name")
