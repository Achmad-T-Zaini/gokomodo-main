...
class assets_report(models.AbstractModel):
    ...
    def get_header(self, options):
        ...
        return [
            [
                {'name': ''},
                {'name': _('Characteristics'), 'class': 'text-center', 'colspan': 6},
#                {'name': _('Assets'), 'class': 'text-center', 'colspan': 4},
                {'name': _('Accumulated Depreciation'), 'class': 'text-center'},
                {'name': _('Net Book Value'), 'class': 'text-center'},
            ],
            [
                {'name': ''},  # Description
                {'name': _('Acquisition Date'), 'class': 'text-center'},  # Characteristics
                {'name': _('Acquisition Amount'), 'class': 'text-center'},  # Additional ODP-15
                {'name': _('Usefull Life'), 'class': 'text-center'},  # Additional ODP-15
                {'name': _('Monthly Depreciation'), 'class': 'text-center'},  # Additional ODP-15
...
#                {'name': _('+'), 'class': 'number'},
#                {'name': _('-'), 'class': 'number'},
                {'name': 'As of ' + end_date, 'class': 'text-center'},
                {'name': 'As of ' + end_date, 'class': 'text-center'},  # Gross
            ],
        ]
    ...
    def _get_lines(self, options, line_id=None):
        ...
        for al in parent_lines:
            ...
            asset_original_value = al['asset_original_value']
            if al['asset_method_period']=='1':
                bulan_period = al['asset_method_number']
            elif al['asset_method_period']=='2':
                bulan_period = al['asset_method_number'] * 12
            else:
                bulan_period = 0

            asset_method_number = str(bulan_period) + ' Months'
            monthly_depreciation = al['depreciation']
            if al['asset_method'] == 'linear':
                monthly_depreciation = asset_original_value / bulan_period
...
#Additional ODP-15
                    {'name': self.format_value(asset_original_value) or '', 'no_format_name': ''},  
                    {'name': asset_method_number or '', 'no_format_name': ''},  
                    {'name': self.format_value(monthly_depreciation) or '', 'no_format_name': ''},  
#End Additional ODP-15
                    {'name': al['asset_date'] and format_date(self.env, al['asset_date']) or '', 'no_format_name': ''},
                    {'name': (al['asset_method'] == 'linear' and _('Straight Line')) or (al['asset_method'] == 'degressive' and _('Declining')) or _('Dec. then Straight'), 'no_format_name': ''},
