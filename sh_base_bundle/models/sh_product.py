...
class ShProductTemplate(models.Model):
    ...
    @api.depends('sh_bundle_product_ids','sh_bundle_product_ids.sh_price_subtotal')
        ...
        for order in self:
            if order.sh_bundle_product_ids:
                ...
                for line in order.sh_bundle_product_ids:
                    ...
                    on_hand_cek = int(float(line.sh_product_id.qty_available) / float(line.sh_qty) )
                    if not on_hand:
                        on_hand = on_hand_cek
                    elif on_hand>on_hand_cek:
                        on_hand = on_hand_cek
                order.write({'sh_amount_total' : amount_total, 'qty_available': on_hand,})
...
class ShBundleProduct(models.Model):
    ...
    sh_price_unit = fields.Float('Unit Price', related="sh_product_id.list_price")
    sh_cost_price = fields.Float(related="sh_product_id.standard_price")
    sh_price_subtotal = fields.Float('Sub Total', readonly=True, store=True, compute="_compute_sub_total")
    ...
    @api.depends('sh_product_id', 'sh_qty', 'sh_price_unit')
    def _compute_sub_total(self):
        for rec in self:
            rec.sh_price_subtotal = rec.sh_price_unit * rec.sh_qty
            rec.sh_bundle_id.compute_bundle_cost_price()
            rec.sh_bundle_id.compute_bundle_price()
